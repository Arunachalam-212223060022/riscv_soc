module cpu_top (
    ...
    pc u_pc (.clk(clk), .rst_n(rst_n), .pc_next(pc_next), .pc_out(pc));
    assign imem_addr = pc;
    ...
    regfile u_rf (
        .clk(clk), .we(reg_write), .rs1(rs1_a), .rs2(rs2_a), .rd(rd_addr),
        .wdata(rd_wdata), .rs1_data(rs1_data), .rs2_data(rs2_data)
    );
    ...
    alu_ctrl u_ac (.alu_op(alu_op), .funct3(funct3), .funct7(funct7), .alu_sel(alu_sel));
    alu      u_alu (.a(rs1_data), .b(alu_b), .alu_sel(alu_sel), .result(alu_result), .zero(zero_flag));
    ...
    always @(*) begin
        case (funct3)
            ...
            3'b100: load_data = {24'b0, dbus_rdata[7:0]};
            3'b101: load_data = {16'b0, dbus_rdata[15:0]};
