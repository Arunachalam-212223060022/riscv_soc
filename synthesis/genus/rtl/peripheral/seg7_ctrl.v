module seg7_ctrl (
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin div_cnt <= 17'd0; tick <= 1'b0; end
        else if (div_cnt == 17'd99999) begin div_cnt <= 17'd0; tick <= 1'b1; end
        else begin div_cnt <= div_cnt + 1; tick <= 1'b0; end
    end

    reg [2:0] digit_sel;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) digit_sel <= 3'd0;
